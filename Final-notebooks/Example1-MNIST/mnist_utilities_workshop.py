...
def ws_read32(bytestream):
    dt = numpy.dtype(numpy.uint32).newbyteorder('>')
    return numpy.frombuffer(bytestream.read(4), dtype=dt)[0]


def ws_extract_images(f):
    ...
    with gzip.GzipFile(fileobj=f) as bytestream:
        magic = ws_read32(bytestream)
        if magic != 2051:
            raise ValueError(
                'Invalid magic number %d in MNIST image file: %s' %
                (magic, f.name))
        num_images = ws_read32(bytestream)
        rows = ws_read32(bytestream)
        cols = ws_read32(bytestream)
    ...
    with gzip.GzipFile(fileobj=f) as bytestream:
        ...
        if magic != 2049:
            raise ValueError(
                ...
                (magic, f.name))
        num_items = ws_read32(bytestream)
        buf = bytestream.read(num_items)
        labels = numpy.frombuffer(buf, dtype=numpy.uint8)
        return labels


def ws_read_data_sets(train_dir, data_type="train"):
    """
...
#        local_file = base.maybe_download(TRAIN_IMAGES, train_dir, SOURCE_URL + TRAIN_IMAGES)
	local_file = train_dir+TRAIN_IMAGES 
        with open(local_file, 'rb') as f:
            train_images = ws_extract_images(f)

#        local_file = base.maybe_download(TRAIN_LABELS, train_dir,
#                                         SOURCE_URL + TRAIN_LABELS)
	local_file = train_dir+TRAIN_LABELS 
        with open(local_file, 'rb') as f:
            train_labels = ws_extract_labels(f)
...
#                                        SOURCE_URL + TEST_IMAGES)
	local_file = train_dir+TEST_IMAGES 
        with open(local_file, 'rb') as f:
            test_images = ws_extract_images(f)

#        local_file = base.maybe_download(TEST_LABELS, train_dir,
#                                         SOURCE_URL + TEST_LABELS)
	local_file = train_dir+TEST_LABELS 
        with open(local_file, 'rb') as f:
            test_labels = ws_extract_labels(f)
